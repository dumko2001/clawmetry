    ...
    if [ -n "$CLUSTER_CONTAINER" ]; then
      ...
      echo "$SANDBOX_NAMES" | while IFS= read -r sb; do
        ...
        if [ -n "$INSTALLED_VER" ]; then
          # Compare versions: upgrade if installed < required
          if python3 -c "from packaging.version import Version; exit(0 if Version('$INSTALLED_VER') >= Version('$REQUIRED_VER') else 1)" 2>/dev/null \
      ...
      if [ -n "$HOST_API_KEY" ] && [ -n "$HOST_ENC_KEY" ]; then
        ...
        echo "$SANDBOX_NAMES" | while IFS= read -r sb; do
          ...
          if [ -n "$SB_KEY" ] && [ "$SB_KEY" = "$HOST_API_KEY" ]; then
            ...
            # Pre-write config so --key matches _saved_api_key (skips OTP check)
            CONNECT_TS=$(date -u +%Y-%m-%dT%H:%M:%S 2>/dev/null || date +%Y-%m-%dT%H:%M:%S)
            docker exec "$CLUSTER_CONTAINER" kubectl exec -n openshell "$sb" -- \
              bash -c "mkdir -p /root/.clawmetry && python3 -c \"
import json
json.dump({'api_key':'$HOST_API_KEY','node_id':'$sb','platform':'Linux','connected_at':'$CONNECT_TS','encryption_key':'$HOST_ENC_KEY'}, open('/root/.clawmetry/config.json','w'))
\"" 2>/dev/null || true

            # Run clawmetry connect non-interactively (OTP skipped since key matches saved config)
            if docker exec "$CLUSTER_CONTAINER" kubectl exec -n openshell "$sb" -- \
