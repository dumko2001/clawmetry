...
def _post(path: str, payload: dict, api_key: str, timeout: int = 45) -> dict:
    ...
    for attempt in range(3):
        try:
            ...
            if code in _RETRYABLE_CODES and attempt < 2:
                wait = 2 ** attempt  # 1s, 2s, (4s — but attempt<2 so max 2s)
                time.sleep(wait)
                continue
            raise last_err
        except Exception as e:
            ...
            if attempt < 2:
                ...
                continue
            break
    log.warning(f"_post({path}) failed after 3 attempts: {last_err}")
    raise last_err
