...
class TestStaleNodes:
    def test_stale_nodes_endpoint_returns_200(self, api, base_url):
        """Stale nodes endpoint returns 200 with expected keys."""
        r = api.get(f"{base_url}/api/nodes/stale", timeout=10)
        assert r.status_code == 200, f"Expected 200, got {r.status_code}: {r.text[:200]}"
        d = r.json()
        assert "stale_nodes" in d, f"Missing 'stale_nodes' key in response: {d}"
        assert "count" in d, f"Missing 'count' key in response: {d}"

    def test_stale_nodes_list_is_list(self, api, base_url):
        """stale_nodes value is a list."""
        r = api.get(f"{base_url}/api/nodes/stale", timeout=10)
        assert r.status_code == 200
        d = r.json()
        assert isinstance(d["stale_nodes"], list), \
            f"stale_nodes should be a list, got {type(d['stale_nodes'])}"

    def test_stale_nodes_count_matches_list(self, api, base_url):
        """count field matches length of stale_nodes list."""
        r = api.get(f"{base_url}/api/nodes/stale", timeout=10)
        assert r.status_code == 200
        d = r.json()
        assert d["count"] == len(d["stale_nodes"]), \
            f"count={d['count']} but list has {len(d['stale_nodes'])} items"
